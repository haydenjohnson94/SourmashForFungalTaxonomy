#!/bin/bash
#SBATCH --job-name=BUSCO_Rerun_Cryptococcus
#SBATCH --output=BUSCO_Rerun_Cryptococcus_%j.out
#SBATCH --error=BUSCO_Rerun_Cryptococcus_%j.err
#SBATCH --mem=120G
#SBATCH --cpus-per-task=16
#SBATCH --partition=YOUR_PARTITION   # set this to your cluster's partition/account

# Run this after DownloadGenomes_Cryptococcus.sh and RunBUSCO_Cryptococcus.sh, to
# retry any BUSCO runs that failed or were interrupted during the initial pass
# (e.g. genomes that need more memory than the per-task allocation provided).
...
for ACCESSION in "${RERUN_ACCESSIONS[@]}"; do
    ...
    if [[ -f "$GENOME_COPY" ]]; then
        ...
        # If the genome file isn't in GENOME_DIR, re-run DownloadGenomes_Cryptococcus.sh
