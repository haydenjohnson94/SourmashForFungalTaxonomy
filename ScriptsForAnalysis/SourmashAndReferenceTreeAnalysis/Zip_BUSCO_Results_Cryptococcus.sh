...
#SBATCH --job-name=Zip_Cryptococcus_32
#SBATCH --output=Zip_Cryptococcus_%j.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --mem=32G

# Packages each BUSCO run's run_fungi_odb10 directory into a single
# run_fungi_odb10_ACCESSION.tar.gz archive, which is the layout
# BuildBUSCOTree_Cryptococcus_Reduced.ipynb expects to read.

# Change to the directory containing all the BUSCO results
cd /path/to/your/analysis_directory/SpeciesComplex_SourmashTrees/CryptococcusComplexes_Reduced/BUSCO_CryptococcusReduced/busco_results
